...
URL=$(perl -n000e 'print $1 while /^..resource\s\"catalog\"\sdo\n\s+url\s\"(.*?)\"\n.*?\n..end\n\n/mg' /tmp/poet.rb)

# remove package resource from poet manifest
perl -0777 -i.original -pe 's/..resource\s\"catalog\"\sdo\n.*?\n.*?\n..end\n\n//igs' /tmp/poet.rb
...
cat > /tmp/pkg.rb <<-EOF
# Homebrew Formula
# catalog, Ian Dennis Miller
# rebuild with 'make homebrew'

class catalog < Formula
  desc "Catalog is a dataset management platform for Python and R"
  homepage "https://github.com/iandennismiller/catalog"
  url "${URL}"
  sha256 "${SHA}"
